...
export default function Detail() {
  ...
  const [propertySelectedRows, setPropertySelectedRows] = useState<Set<number>>(new Set());
  ...
  const [billingSelectedRows, setBillingSelectedRows] = useState<Set<number>>(new Set());
...
useEffect(() => {
  ...
  const uniqueOwnerships = [...new Set(allPropertyData.map((item) => item["Ownership"]))];
  const uniquePropertyStatuses = [...new Set(allPropertyData.map((item) => item["Status"]))];
  const uniqueBillingStatuses = [...new Set(allBillingData.map((item) => item["Status"]))];
  ...
  const filteredPropertyData = useMemo(() => {
    ...
    if (propertySearchTerm) {
      filtered = filtered.filter(
        (item) =>
          item["Property Name"].toLowerCase().includes(propertySearchTerm.toLowerCase()) ||
          item["Application ID"].toLowerCase().includes(propertySearchTerm.toLowerCase()) ||
          item["Ownership"].toLowerCase().includes(propertySearchTerm.toLowerCase()) ||
          item["Reviewed By"].toLowerCase().includes(propertySearchTerm.toLowerCase())
      );
    }

    if (propertyFilters.ownership) {
      filtered = filtered.filter((item) => item["Ownership"] === propertyFilters.ownership);
    }

    if (propertyFilters.status) {
      filtered = filtered.filter((item) => item["Status"] === propertyFilters.status);
    }
  ...
  const filteredBillingData = useMemo(() => {
    ...
    if (billingSearchTerm) {
      filtered = filtered.filter(
        (item) =>
          item["Plan Name"].toLowerCase().includes(billingSearchTerm.toLowerCase()) ||
          item["Amount"].toLowerCase().includes(billingSearchTerm.toLowerCase())
      );
    }

    if (billingFilters.status) {
      filtered = filtered.filter((item) => item["Status"] === billingFilters.status);
    }
  ...
  const handlePropertySelectAll = (checked: boolean) => {
    ...
    if (checked) {
      filteredPropertyData.forEach((item) => newSelected.add(item.id));
    } else {
      filteredPropertyData.forEach((item) => newSelected.delete(item.id));
    }
  ...
  const isAllPropertySelected = useMemo(() => {
    return (
      ...
      filteredPropertyData.every((item) => propertySelectedRows.has(item.id))
    );
  }, [filteredPropertyData, propertySelectedRows]);

  const isSomePropertySelected = useMemo(() => {
    return (
      filteredPropertyData.some((item) => propertySelectedRows.has(item.id)) &&
  ...
  const handleBillingSelectAll = (checked: boolean) => {
    ...
    if (checked) {
      filteredBillingData.forEach((item) => newSelected.add(item.id));
    } else {
      filteredBillingData.forEach((item) => newSelected.delete(item.id));
    }
  ...
  const isAllBillingSelected = useMemo(() => {
    return (
      ...
      filteredBillingData.every((item) => billingSelectedRows.has(item.id))
    );
  }, [filteredBillingData, billingSelectedRows]);

  const isSomeBillingSelected = useMemo(() => {
    return (
      filteredBillingData.some((item) => billingSelectedRows.has(item.id)) &&
      !isAllBillingSelected
    );
  }, [filteredBillingData, billingSelectedRows, isAllBillingSelected]);

  // Delete Handlers
  const openDeleteSingleModal = (row: Record<string, string>, id: number, type: "property" | "billing") => {
  ...
  const handleModalConfirm = () => {
    if (modalType === "multiple") {
      if (deleteType === "property" && propertySelectedRows.size > 0) {
        const idsToDelete = Array.from(propertySelectedRows);
        const updatedData = allPropertyData.filter((item) => !idsToDelete.includes(item.id));
        setAllPropertyData(updatedData);
        setPropertySelectedRows(new Set());
      } else if (deleteType === "billing" && billingSelectedRows.size > 0) {
        const idsToDelete = Array.from(billingSelectedRows);
        const updatedData = allBillingData.filter((item) => !idsToDelete.includes(item.id));
        setAllBillingData(updatedData);
        setBillingSelectedRows(new Set());
      }
    } else if (modalType === "single" && singleRowToDelete) {
      if (singleRowToDelete.type === "property") {
        const updatedData = allPropertyData.filter((item) => item.id !== singleRowToDelete.id);
        setAllPropertyData(updatedData);
        const newSelected = new Set(propertySelectedRows);
        newSelected.delete(singleRowToDelete.id);
        setPropertySelectedRows(newSelected);
      } else {
        const updatedData = allBillingData.filter((item) => item.id !== singleRowToDelete.id);
        setAllBillingData(updatedData);
        const newSelected = new Set(billingSelectedRows);
        newSelected.delete(singleRowToDelete.id);
        setBillingSelectedRows(newSelected);
    ...
    <div className="">
      ...
      <div className="mt-8">
        <Table
          ...
          isDeleteAllDisabled={propertySelectedRows.size === 0 || propertySelectedRows.size < displayPropertyData.length}
        />
      </div>
      ...
      <div className="mt-10">
        <Table
          ...
          isDeleteAllDisabled={billingSelectedRows.size === 0 || billingSelectedRows.size < displayBillingData.length}
        />
      </div>
