...
export default function Applications() {
  ...
  const handleSelectAll = (checked: boolean) => {
    setSelectedRows(new Set(checked ? currentData.map(item => item.id) : []));
  };
  ...
  const isAllDisplayedSelected = currentData.length > 0 && currentData.every(item => selectedRows.has(item.id));
  const isSomeDisplayedSelected = currentData.some(item => selectedRows.has(item.id)) && !isAllDisplayedSelected;
